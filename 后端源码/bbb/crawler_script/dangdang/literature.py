...
class Literature():
    ...
    @classmethod
    def literature_spider(cls, app, ip):
        page = 1

        db = dang_model.db
        db.init_app(app)
        pra = dang_model.Literature.query.delete()
        print(pra)
        db.session.commit()
        while True:
            ua = UserAgent()
            ...
            req = requests.get('http://bang.dangdang.com/books/bestsellers/01.05.00.00.00.00-24hours-0-0-1-%d'%page,
            ...
            for i in range(len(name)):
                best = dang_model.Literature(rank=id[i], name=name[i], bo_na=bo_na[i], url=url[i],
