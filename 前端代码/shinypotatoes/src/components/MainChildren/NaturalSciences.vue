...
export default {
  ...
  methods: {
    get_dat () {
      this.$axios.get('/dangdang/natural_sciences',
        {
          params: { page4: this.page4 }
        }).then((req) => {
  ...
  mounted () {
    window.addEventListener('scroll', this.scroll4(), true)
  },
  destroyed () {
    window.removeEventListener('scroll', this.scroll4(), true)
  }
}
</script>
